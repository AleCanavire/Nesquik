        ...
        <div className="title-detail-modal" style={showDetail ? {} : {opacity: "0", transform: "scale(0.8)"}} role="dialog" aria-modal="true">
          <div className="title-detail-header">
            ...
            <div className="title-logo-and-buttons">
              { infoTitle?.logo?.file_path &&
                <img
                  className="title-logo"
                  src={`https://image.tmdb.org/t/p/w300${infoTitle.logo.file_path}`}
                />
              }
              ...
              <div className="metadata-right">
                { metadata?.cast?.length > 0 &&
                  <div className="title-tags">
                    ...
                    { metadata.cast.map((actor, index) => {
                      return(
                        <React.Fragment key={index}>
                          <span className="title-tag-item">{actor}</span>
                          <>, </>
                        </React.Fragment>
                      )})
                    }
                  ...
                  <div className="title-tags">
                    ...
                    { metadata.genres.map((genre, index) => {
                      return(
                        <React.Fragment key={index}>
                          {index !== 0 ?  <>, </> : null}
                          <span className="title-tag-item">{genre}</span>
                        </React.Fragment>
                      )})
                    }
                  </div>
                }
                { metadata?.crew?.length > 0 &&
                  <div className="title-tags">
                    <span className="title-tag-label">Creado por: </span>
                    { metadata.crew.map((person, index) => {
                      return(
                        <React.Fragment  key={index}>
                          {index !== 0 ?  <>, </> : null}
                          <span className="title-tag-item">{person}</span>
                        </React.Fragment>
                      )})
                    }
                  </div>
                  ...
                  <div className="about-tag">
                    ...
                    { metadata.crew.map((person, index) => {
                      return(
                        <React.Fragment key={index}>
                          {index !== 0 ?  <>, </> : null}
                          <span className="tag-item">{person}</span>
                        </React.Fragment>
                      )})
                    }
                  </div>
